...
Page(Object.assign({}, swiperAutoHeight, {
  ...
  getData() {
    ...
    new Census(res => {
      ...
      if (res.body.ranking) {
        if (res.body.ranking.username && res.body.ranking.username.length > 10) {
      ...
      if (res.body.rankingList) {
        for (var i = 0; i < res.body.rankingList.length; i++) {
          // console.log(res.body.rankingList[i].username)
      ...
      if (res.body.runInfos) {
        for (var i = 0; i < res.body.runInfos.length; i++) {
          if (res.body.runInfos[i] !== null){
            if (res.body.runInfos[i].username && res.body.runInfos[i].username.length > 10) {
              res.body.runInfos[i].username = res.body.runInfos[i].username.substr(0, 3) + '···' + res.body.runInfos[i].username.substr(res.body.runInfos[i].username.length - 3, 3)
            }
          }
