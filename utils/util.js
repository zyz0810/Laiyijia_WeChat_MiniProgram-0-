...
function errShow(text, duration = 2000, callback = function() {}) {
...
/**
 * getUrlAfterUpload选择图片上传
 * fn上传成功函数，failChooseFn选择图片取消，failFn上传图片失败，url上传地址，type图片类型（image：图片，idcard：身份证）
 */
function getUrlAfterUpload(fn, failChooseFn, failFn, url, type) {
  var url = url ? url : config.BASE_URL + 'api/v1/file/uploadSingleImage'
  wx.chooseImage({
    count: 1, // 默认9
    ...
    success: function(res) {
      ...
      wx.uploadFile({
        ...
        success: function(res) {
          ...
          // var data = res.data
          if (data.head.returnCode == 0 && data.head.subCode == 0) {
            if (fn) fn(data.body.picUrl, tempFilePaths)
          } else if (data.head.returnCode != 0 && data.head.subCode == 0) {
            // util.errShow(res.data.head.returnMsg)
            wx.showToast({
              title: res.data.head.returnMsg,
              icon: 'none'
            })
          } else if (data.head.returnCode == 0 && data.head.subCode != 0) {
            // util.errShow(res.data.head.subMsg)
            wx.showToast({
              title: res.data.head.subMsg,
              icon: 'none'
            })
          } else if (data.head.returnCode != 0 && data.head.subCode != 0) {
            // util.errShow(res.data.head.returnMsg)
        ...
        fail: function(res) {
          wx.hideLoading()
          if (failFn) failFn(res, tempFilePaths)
          errShow('上传失败！服务器或网络错误')
        }
      })
    },
    fail: function(res) {
      wx.hideLoading()
      errShow('未选择图片', 500)
      if (failChooseFn) failChooseFn(res)
    }
  })
}
...
// html 中: num = 0 为本月 / 本周，  上月 / 上周 按钮： 《   每点击一次就 num += 1，时间即为getAppointedDate(2, num); 下月 / 下周 按钮：》  num -= 1
...
function getAppointedDate(optType, optPageNum) {　　
  ...
  if (optType == 1) {　　　　
    ...
    endDate = formatDateFn(new Date(nowYear, lastMonth + 1 - (1 * optPageNum), 
    getMonthDays(nowYear, lastMonth + 1 - (1 * optPageNum))))　　
  }　　
  return {
    startDate: startDate,
    endDate: endDate
